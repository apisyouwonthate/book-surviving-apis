...
You may well have heard developers saying: _don't let tests hit external dependencies!_

Some folks take that to various different extremes, and don't even let their
tests talk to a database they control. That might make sense in some situations
...
just going to happen, but that's not the case! There is a tactic called "record and replay",
...
mock an HTTP server. They are also very different from API specification based
mocking tools like http://github.com/stoplightio/prism/[Prism], which will be
discussed a bit later.

Web mocking tools can be configured to respond in certain ways based on
the URL, HTTP method, or body params which are used. Most of the time this is used for simple stuff.
...
- **Node:** https://github.com/nock/nock[Nock]
...
properly. Maybe send them a copy of _Build APIs You Won't Hate._ if they need convincing.
