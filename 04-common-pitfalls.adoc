...
goes wrong. It would be like me thinking "I can just ride my bike to work in NYC
without two taxi drivers, a truck, a family unit of tourists, cops on horses, a
dog walker, and a raccoon all trying to kill me."

There are so many assumptions being made when talking to another API that it's
hard to count them all.

.The most basic API interaction possible, fetching some JSON data from the PokéAPI to show all of Squirtle's abilities.
[,javascript]
----
include::code/ch-05-common-pitfalls/01-happy-path-only.js[]
----

This is assuming that the request is going to get a response, in a reasonable time-frame (no definition of what that is), and when it does it's actually talking to the server you want to talk to and not being hijacked by Wifi, so they feel confident assuming that JSON will be returned the PokeAPI said it would?

Assuming server exists, responds in a decent time, and the data is in fact JSON, there's a few more issues in that we've presumed the pokemon was definitely found (not typos or missing data), and confidently tied our code to an exact JSON structure with no checks at all.
...
Let's look into what some of those things are, so you can code in a way that defends your API client, and your precious time, from all the nonsense that can go wrong.
